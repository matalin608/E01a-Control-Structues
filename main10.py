...
while (play_again != 'n' and play_again != 'no'):
    #this sets a condition where when play_again is not equal to 'n' and 'no', then the following lines become relevant
    match_color = random.choice(colors)
    #match_color returns a random item from the colors list
    count = 0
    color = ''
    while (color != match_color):
        #this sets a condition where when color does not equal a random choice from the colors list, the following lines become relevant.
        ...
        count += 1
        #this adds 1 to the running count for every input of an answer for "color"
    ...
    #this will add a line at the end which gives the number of entries a person had made qhile guessing the correct color. this uses the variable count just as the line before it did.
    if (count < best_count):
        print('This was your best guess so far!') 
        best_count = count
    play_again = input("\nWould you like to play again? ").lower().strip()
    #this line calls back to line 17, indicating that, unless the answer entered is "n" or "no", the program will begin again. 
